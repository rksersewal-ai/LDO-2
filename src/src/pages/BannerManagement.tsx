...
import { Megaphone, Plus, Edit3, Trash2, Eye, X, GripVertical, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl text-white mb-2" style={{ fontWeight: 700 }}>Announcement Management</h1>
          <p className="text-slate-400 text-sm">Create and manage running banner announcements visible to all users.</p>
        </div>
        <Button onClick={() => { setEditBanner(null); setShowForm(true); }}><Plus className="w-4 h-4" /> New Announcement</Button>
      </div>

      {/* Preview */}
      <GlassCard className="p-4">
        <h3 className="text-xs text-slate-500 uppercase tracking-widest mb-3" style={{ fontWeight: 700 }}>Live Preview</h3>
          ...
          <div className="flex-1 overflow-hidden">
            <div className="flex gap-8 animate-marquee">
              {banners.filter(b => b.active).map(b => (
                <span key={b.id}><span style={{ fontWeight: 600 }}>{b.title}:</span> {b.message}</span>
              ))}
              {banners.filter(b => b.active).length === 0 && <span className="text-teal-300/50">No active announcements</span>}
            </div>
          </div>
        </div>
      </GlassCard>
      ...
      <GlassCard className="p-0 overflow-hidden">
        <div className="divide-y divide-slate-800/50">
          {banners.map((b, i) => (
            <div key={b.id} className="flex items-center gap-4 p-4 hover:bg-slate-800/20 transition-colors">
              <GripVertical className="w-4 h-4 text-slate-600 cursor-grab" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  <h3 className="text-sm text-slate-200" style={{ fontWeight: 600 }}>{b.title}</h3>
                ...
                <div className="flex items-center gap-3 text-[10px] text-slate-500 mt-1">
                  ...
                  <span>Valid: {b.validFrom} → {b.validTo}</span>
        ...
        {banners.length === 0 && <div className="py-16 text-center text-slate-500">No announcements configured.</div>}
      </GlassCard>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowForm(false)}>
          <GlassCard className="w-full max-w-lg p-6" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-sm text-slate-300 mb-1">Message</label><textarea className="w-full bg-slate-950/50 border border-teal-500/20 text-slate-200 text-sm rounded-xl px-4 py-2 focus:outline-none focus:border-teal-400/50 h-24 resize-none" defaultValue={editBanner?.message} placeholder="Announcement message..." /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm text-slate-300 mb-1">Valid From</label><Input type="date" className="w-full" defaultValue={editBanner?.validFrom} /></div>
                <div><label className="block text-sm text-slate-300 mb-1">Valid To</label><Input type="date" className="w-full" defaultValue={editBanner?.validTo} /></div>
              </div>
              <div><label className="block text-sm text-slate-300 mb-1">Link Target (optional)</label><Input className="w-full" defaultValue={editBanner?.link || ''} placeholder="/documents or URL" /></div>
              <div className="flex gap-3 pt-2">
                <Button variant="secondary" className="flex-1" onClick={() => setShowForm(false)}>Cancel</Button>
                <Button className="flex-1" onClick={() => setShowForm(false)}>{editBanner ? 'Update' : 'Create'}</Button>
              </div>
            </div>
          </GlassCard>
        </div>
