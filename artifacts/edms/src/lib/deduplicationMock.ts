...
export const DUPLICATE_GROUPS: DuplicateGroup[] = [
  {
    ...
    risks: ['Two documents are archived copies of the same released revision.', 'One duplicate is referenced in an old approval pack and should be hidden, not deleted.'],
    notes: 'Operations requested the master to remain the design-office upload because it carries the cleanest approval lineage.',
    decisionLog: [
      { at: '2026-03-09 11:12', actor: 'records.admin', action: 'Fingerprint confirmed', note: 'Sparse fingerprint matched all four files; one full-file hash was already present.' },
    ],
    documents: [
      {
        id: 'DOC-2026-9301',
  ...
  },
  {
    ...
    risks: ['Only metadata currently matches; two files still need sparse fingerprints.', 'Vendor qualification packs often bundle annexures with the same title but different attachments.'],
    notes: 'Keep review conservative until missing fingerprints are scanned for the imported copies.',
    decisionLog: [
      { at: '2026-03-12 09:08', actor: 'quality.admin', action: 'Pending review', note: 'Grouped by supplier code, title, revision, and near-identical file size.' },
    ],
    documents: [
      {
        id: 'DOC-2026-9311',
  ...
  },
  {
    ...
    dedupModeUsed: 'fingerprint',
    risks: ['Revisions B.3 and B.4 are mixed in the same candidate set.', 'Configuration links point to two active BOM items, so merge requires master confirmation.'],
    notes: 'Pending confirmation because metadata is close but the latest revision may supersede an older released interface spec.',
    decisionLog: [
      { at: '2026-03-19 16:44', actor: 'records.admin', action: 'Needs confirmation', note: 'Sparse fingerprints differ slightly; full hash recommended only if the group is promoted for action.' },
    ],
    documents: [
      {
        id: 'DOC-2026-9321',
  ...
  },
  {
    ...
    dedupModeUsed: 'fingerprint',
    risks: ['One archived maintenance pack is still referenced by a closed work record.', 'No revision mismatch; safe to hide archived copies after note capture.'],
    notes: 'Recommended action is hide duplicates and keep the newest design-office upload as master.',
    decisionLog: [
      { at: '2026-03-08 08:55', actor: 'records.admin', action: 'Exact duplicate', note: 'Fingerprints and file size match; archived copy can be hidden after link review.' },
    ],
    documents: [
      {
        id: 'DOC-2026-9331',
