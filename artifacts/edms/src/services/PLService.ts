...
import type { EngineeringChange, InspectionCategory, PLNumber, PLStatus, SafetyClassification } from '../lib/types';
...
function mapApiPlItem(item: any): PLNumber {
  const status = item.status === 'ACTIVE' || item.status === 'UNDER_REVIEW' || item.status === 'OBSOLETE'
    ? item.status
    : item.status === 'Retired'
      ? 'OBSOLETE'
  ...
  return {
    ...
    safetyClassification: item.safety_classification as SafetyClassification | undefined,
    ...
    drawingNumbers: Array.isArray(item.drawing_numbers) ? item.drawing_numbers : [],
    ...
    recentActivity: Array.isArray(item.recent_activity) ? item.recent_activity : [],
    engineeringChanges: Array.isArray(item.engineering_changes) ? item.engineering_changes : [],
    linkedDocumentIds: Array.isArray(item.linked_document_ids) ? item.linked_document_ids : [],
    linkedWorkIds: Array.isArray(item.linked_work_ids) ? item.linked_work_ids : [],
    linkedCaseIds: Array.isArray(item.linked_case_ids) ? item.linked_case_ids : [],
...
export const PLService = {
  ...
  async getById(id: string): Promise<PLNumber | null> {
    if (useMockApi) {
      ...
      return mockStore.find(p => p.id === id || p.plNumber === normalized || p.plNumber === id) ?? null;
    }
    try {
  ...
  async update(id: string, patch: Partial<PLNumber>): Promise<PLNumber | null> {
    if (useMockApi) {
      ...
      mockStore[idx] = { ...mockStore[idx], ...patch, updatedAt: new Date().toISOString().split('T')[0] };
      return mockStore[idx];
    }

    const normalizedId = id.replace('PL-', '');
    const { linkedDocumentIds, ...metadataPatch } = patch;
    const updated = await apiClient.updatePlItem(normalizedId, toApiPayload(metadataPatch));
  ...
  async addEngineeringChange(
    plId: string,
    ec: { ecNumber: string; description: string; status: string; date: string; author?: string }
  ...
  async setDocumentLinks(plId: string, documentIds: string[]): Promise<PLNumber> {
    if (useMockApi) {
      const updated = await this.update(plId, { linkedDocumentIds: documentIds });
      if (!updated) {
    ...
    const response = await apiClient.client.post(`/pl-items/${plId.replace('PL-', '')}/documents/set/`, {
