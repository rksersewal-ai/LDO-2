...
function loadStore() {
  if (typeof window === 'undefined') {
    return DEFAULT_TEMPLATES.map((template) => ({ ...template, fields: cloneFields(template.fields), tags: [...template.tags] }));
  }

  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) {
      const seeded = DEFAULT_TEMPLATES.map((template) => ({ ...template, fields: cloneFields(template.fields), tags: [...template.tags] }));
      persist(seeded);
      return seeded;
    }
    ...
    const fallback = DEFAULT_TEMPLATES.map((template) => ({ ...template, fields: cloneFields(template.fields), tags: [...template.tags] }));
    persist(fallback);
    return fallback;
  }
}
...
export const DocumentTemplateService = {
  ...
  async create(input: CreateTemplateInput) {
    const template: DocumentTemplateRecord = {
      ...
      fields: input.fields && input.fields.length > 0 ? cloneFields(input.fields) : [{ label: 'Document Title', type: 'text', required: true }],
    };
  ...
  async toggleStar(id: string) {
    _store = _store.map((template) => template.id === id ? { ...template, starred: !template.starred } : template);
    persist(_store);
  ...
  async recordUsage(id: string) {
    ...
    _store = _store.map((template) => template.id === id
