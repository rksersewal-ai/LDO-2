...
function getFingerprintState(document: BackendDocument): DuplicateCandidateDocument['fingerprintState'] {
...
function getPatternHits(document: BackendDocument, key: string) {
  const patterns = document.search_metadata && typeof document.search_metadata === 'object'
...
function deriveGroupStatus(groupKey: string, candidates: DuplicateCandidateDocument[]): GroupStatus {
  if (groupKey.startsWith('full:')) return 'exact';
  if (candidates.some((document) => document.fingerprintState === 'missing')) return 'pending';
  return 'probable';
}

function deriveRisks(documents: BackendDocument[], candidates: DuplicateCandidateDocument[], status: GroupStatus) {
  const risks: string[] = [];
  const revisions = new Set(documents.map((document) => document.revision ?? 1));
  if (revisions.size > 1) {
    risks.push('Multiple revisions are present in the same duplicate family; verify that release lineage remains intact.');
  }
  if (candidates.some((candidate) => candidate.isArchived)) {
    risks.push('At least one candidate is obsolete/archived and should be hidden rather than deleted.');
  }
  if (status !== 'exact') {
    risks.push('This group should be treated as review-first because full-file confirmation is not available for every candidate.');
  }
  if (!risks.length) {
    risks.push('Stored metadata and fingerprints agree across this group; confirm links before applying a destructive decision.');
  }
  return risks;
}
...
function buildGroupFromDocuments(groupKey: string, documents: BackendDocument[], index: number): DuplicateGroup {
  ...
  return {
    ...
    dedupModeUsed: groupKey.startsWith('full:') || groupKey.startsWith('sparse:') ? 'fingerprint' : ('metadata' as DedupMode),
    risks: deriveRisks(documents, candidates, status),
    notes: 'Loaded from stored duplicate metadata and indexed fingerprints already present on the document records.',
...
function mapBackendGroup(group: BackendDuplicateGroup, index: number): DuplicateGroup {
  ...
  const categories = Array.isArray(group.categories) && group.categories.length > 0 ? group.categories.map(toLabel) : base.classSummary;
  const sources = Array.isArray(group.source_systems) && group.source_systems.length > 0 ? group.source_systems.map(mapSourceSystem).join(', ') : undefined;
...
async function fetchDuplicateDocuments(signal?: AbortSignal) {
  ...
  while (hasMore && page <= 10) {
    ...
    documents.push(...(response.items as BackendDocument[]));
...
export const DeduplicationService = {
  async getCandidateGroups(signal?: AbortSignal): Promise<DeduplicationLoadResult> {
    try {
      const backendGroups = await apiClient.getDeduplicationGroups(undefined, { signal });
      const groups = backendGroups.map((group: BackendDuplicateGroup, index: number) => mapBackendGroup(group, index));
      if (groups.length > 0) {
        return {
          groups,
          source: 'backend',
      ...
      console.warn('[DeduplicationService] Backend duplicate lookup failed, using fallback console dataset.', error);
    }

    return {
      groups: DUPLICATE_GROUPS,
      source: 'mock',
      summary: 'Using the console fallback dataset because the backend duplicate queue is not populated yet.',
    };
  },

  async applyDecision(groupKey: string, payload: { decision: DedupDecision; masterDocumentId?: string; notes?: string }) {
    if (payload.decision === 'ignore_for_now') {
      return apiClient.ignoreDeduplicationGroup(groupKey, { notes: payload.notes });
    }
