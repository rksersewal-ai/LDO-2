...
import { BookOpen, FileText, Search, Plus, Star, StarOff, ArrowRight, Tag, X, Trash2 } from 'lucide-react';
import { toast } from 'sonner';
import { GlassCard, Button, Input, PageHeader, Select } from '../components/ui/Shared';
...
export default function DocumentTemplates() {
  ...
  const categories = useMemo(
    () => ['All', ...Array.from(new Set([...CATEGORY_SEED, ...templates.map((template) => template.category)]))],
    [templates]
  );
  ...
  function addTemplateField() {
    setDraftTemplate((current) => ({ ...current, fields: [...current.fields, createEmptyField()] }));
  }

  function removeTemplateField(index: number) {
    setDraftTemplate((current) => ({
      ...current,
      fields: current.fields.length > 1 ? current.fields.filter((_, fieldIndex) => fieldIndex !== index) : current.fields,
    }));
  }
  ...
  async function createTemplate() {
    ...
    const invalidSelectField = sanitizedFields.find((field) => field.type === 'select' && (!field.options || field.options.length === 0));
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <PageHeader title="Document Templates" subtitle="Start a document from a reusable template, then continue into ingest with the captured context.">
        <Button size="sm" className="flex items-center gap-2" onClick={() => setShowCreateModal(true)}>
          ...
          <GlassCard className="w-full max-w-3xl p-6 relative max-h-[90vh] overflow-y-auto" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-white">Template Fields</h4>
        ...
        >
          <GlassCard className="w-full max-w-lg p-6 relative" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
              {previewTemplate.fields.map((field) => (
                <div key={field.label}>
                  ...
                  {field.type === 'select' ? (
                    <Select value={formValues[field.label] || ''} onChange={(event) => setFormValues((current) => ({ ...current, [field.label]: event.target.value }))}>
                    ...
                    <DatePicker
                      ...
                      onChange={(nextValue) => setFormValues((current) => ({ ...current, [field.label]: nextValue }))}
                      placeholder="Select date"
                    />
                  ) : (
                    <Input
                      value={formValues[field.label] || ''}
                      onChange={(event) => setFormValues((current) => ({ ...current, [field.label]: event.target.value }))}
            ...
            <div className="flex gap-3 mt-5">
              <Button onClick={submitForm} className="flex-1">Continue to Ingest</Button>
              <Button variant="ghost" onClick={closeUseModal}>Cancel</Button>
            </div>
          </GlassCard>
        </div>
      )}
...
function TemplateCard({
  ...
  return (
    <GlassCard className="p-5 flex flex-col gap-3 hover:border-teal-500/30 transition-all group">
      ...
      <Button onClick={() => onUse(template)} size="sm" className="w-full flex items-center justify-center gap-2 mt-1">
