...
import {
  Server, Database, Cpu, HardDrive, Activity, CheckCircle, AlertTriangle,
...
const SERVICES = [
  { name: 'EDMS Web App', status: 'healthy', uptime: '99.98%', latency: '42ms', lastCheck: '5s ago' },
  { name: 'API Server', status: 'healthy', uptime: '99.95%', latency: '67ms', lastCheck: '5s ago' },
  { name: 'OCR Pipeline', status: 'warning', uptime: '97.2%', latency: '1.4s', lastCheck: '5s ago' },
  { name: 'Search Index', status: 'healthy', uptime: '99.99%', latency: '12ms', lastCheck: '5s ago' },
  { name: 'Auth Service', status: 'healthy', uptime: '100%', latency: '8ms', lastCheck: '5s ago' },
  { name: 'Backup Service', status: 'healthy', uptime: '99.7%', latency: '—', lastCheck: '10m ago' },
];

const SLOW_QUERIES = [
  { query: 'SELECT * FROM documents WHERE ocr_status = "Processing"', duration: '1840ms', hits: 42 },
  { query: 'JOIN work_records ON pl_number LIKE "%WAP%"', duration: '920ms', hits: 18 },
  { query: 'COUNT(*) FROM audit_log GROUP BY user_id', duration: '640ms', hits: 7 },
];

const BACKUPS = [
  { label: 'Last Full Backup', time: '2026-03-25 02:00 UTC', size: '1.4 GB', status: 'success' },
  { label: 'Last Incremental', time: '2026-03-25 08:00 UTC', size: '42 MB', status: 'success' },
  { label: 'Next Scheduled', time: '2026-03-25 14:00 UTC', size: '~50 MB', status: 'pending' },
];
          ...
          <GlassCard key={chart.label} className="p-4">
            ...
            <ResponsiveContainer width="100%" height={80}>
              <AreaChart data={chart.data}>
                ...
                <Tooltip contentStyle={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '11px' }} />
                <Area type="monotone" dataKey="value" stroke={chart.color} fill={`url(#grad-${chart.color})`} strokeWidth={1.5} />
              </AreaChart>
            </ResponsiveContainer>
          </GlassCard>
        ))}
