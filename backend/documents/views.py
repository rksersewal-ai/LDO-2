...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='ingest', parser_classes=(MultiPartParser, FormParser))
    def ingest(self, request):
        serializer = DocumentIngestSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        payload = DocumentService.ingest(serializer.validated_data, request.user, request)
        return Response(
            {
                'document': DocumentSerializer(payload['document'], context={'request': request}).data,
    ...
    @action(detail=True, methods=['post'], url_path=r'entities/(?P<entity_pk>[^/.]+)/approve')
    ...
    @action(detail=True, methods=['post'], url_path=r'entities/(?P<entity_pk>[^/.]+)/reject')
    ...
    @action(detail=True, methods=['post'], url_path=r'entities/(?P<entity_pk>[^/.]+)/promote')
    def promote_entity(self, request, pk=None, entity_pk=None):
        field_key = str(request.data.get('field_key', '')).strip()
        if not field_key:
            return Response({'field_key': ['field_key is required.']}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(DocumentMetadataAssertionSerializer(assertion).data, status=status.HTTP_201_CREATED)
    ...
    @action(detail=True, methods=['post'], url_path=r'assertions/(?P<assertion_pk>[^/.]+)/approve')
    ...
    @action(detail=True, methods=['post'], url_path=r'assertions/(?P<assertion_pk>[^/.]+)/reject')
...
class OcrJobViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = PermissionService.scope_queryset(super().get_queryset(), self.request.user, 'view_ocrjob')
        status_filter = self.request.query_params.get('status')
        if status_filter:
            queryset = queryset.filter(status=status_filter)
        search = self.request.query_params.get('search')
        if search:
            queryset = queryset.filter(Q(document__name__icontains=search) | Q(document__id__icontains=search))
        return queryset
...
class OcrResultView(APIView):
    ...
    def get(self, request, document_id):
        return Response(OcrApplicationService.result_for_document(document_id, request.user))
...
class IndexedSourceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = PermissionService.scope_queryset(super().get_queryset(), self.request.user, 'view_indexedsource')
    ...
    def perform_create(self, serializer):
        source = IndexedSourceService.create_or_update(serializer.validated_data, self.request)
        serializer.instance = source

    def create(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        source = IndexedSourceService.create_or_update(serializer.validated_data, request)
...
class CrawlJobViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = PermissionService.scope_queryset(super().get_queryset(), self.request.user, 'view_crawljob')
...
class HashBackfillJobViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = PermissionService.scope_queryset(super().get_queryset(), self.request.user, 'view_hashbackfilljob')
        source = self.request.query_params.get('source')
        if source:
            queryset = queryset.filter(source_id=source)
        status_filter = self.request.query_params.get('status')
...
class DuplicateDecisionViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = DuplicateDecision.objects.select_related('master_document', 'decided_by').all()
    serializer_class = DuplicateDecisionSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        queryset = PermissionService.scope_queryset(super().get_queryset(), self.request.user, 'view_duplicatedecision')
        status_filter = self.request.query_params.get('status')
        if status_filter:
            queryset = queryset.filter(status=status_filter.upper())
        return queryset
...
class DeduplicationGroupListView(APIView):
    ...
    def get(self, request):
        groups = DuplicateDecisionService.group_documents(request.query_params, request.user)
...
class DeduplicationGroupDetailView(APIView):
    ...
    def get(self, request, group_key):
        ...
        if not group:
            return Response({'detail': 'Duplicate group not found'}, status=status.HTTP_404_NOT_FOUND)
...
class DeduplicationGroupDecisionView(APIView):
    ...
    def post(self, request, group_key):
        ...
        return Response(DuplicateDecisionSerializer(record).data, status=status.HTTP_200_OK)
