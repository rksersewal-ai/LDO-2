...
from documents.models import CrawlJob, DocumentMetadataAssertion, DocumentOcrEntity, DuplicateDecision, HashBackfillJob, IndexedSource
...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    def get_revision_label(self, obj):
        ...
        return ingest_metadata.get('revision_label') or metadata.get('revision_label') or str(obj.revision)
...
class DocumentIngestSerializer(serializers.Serializer):
    ...
    revision_label = serializers.CharField(max_length=50, required=False, allow_blank=True)
    ...
    def validate_template_fields(self, value):
        ...
        if isinstance(value, str):
            try:
                ...
                raise serializers.ValidationError('template_fields must be valid JSON.') from exc
...
class OcrJobSerializer(serializers.ModelSerializer):
    ...
    def get_page_count(self, obj):
        return obj.document.ocr_pages.count() if hasattr(obj.document, 'ocr_pages') else 0

    def get_entity_count(self, obj):
        return obj.document.ocr_entities.count() if hasattr(obj.document, 'ocr_entities') else 0
...
class IndexedSourceSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        read_only_fields = ['id', 'created_by', 'last_crawled_at', 'last_successful_crawl_at', 'last_error', 'created_at', 'updated_at']
...
class CrawlJobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'requested_by', 'status', 'discovered_count', 'indexed_count', 'duplicate_count', 'failed_count', 'started_at', 'completed_at', 'error_message', 'created_at', 'updated_at']
...
class HashBackfillJobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'requested_by', 'status', 'documents_scanned', 'documents_indexed', 'full_hashes_computed', 'started_at', 'completed_at', 'error_message', 'created_at', 'updated_at']


class DuplicateDecisionSerializer(serializers.ModelSerializer):
    master_document_id = serializers.CharField(source='master_document.id', read_only=True, allow_null=True)
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'status', 'candidate_documents', 'candidate_count', 'storage_saved_bytes', 'decided_by', 'decided_at', 'created_at', 'updated_at']
...
class DocumentMetadataAssertionCreateSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(choices=['PROPOSED', 'APPROVED'], required=False, default='APPROVED')


class DuplicateGroupSerializer(serializers.Serializer):
    group_key = serializers.CharField()
    family_key = serializers.CharField(allow_blank=True, allow_null=True, required=False)
    ...
    revision_labels = serializers.ListField(child=serializers.CharField(), required=False)
    approved_assertions = serializers.ListField(child=serializers.DictField(), required=False)
    conflicting_entities = serializers.ListField(child=serializers.DictField(), required=False)
