...
from documents.services import CrawlJobService, DocumentOcrProcessingService, HashBackfillJobService
...
def run_indexed_source_crawl(self, job_or_source_id, force_full_hash=False):
    ...
    if source is None:
        raise CrawlJob.DoesNotExist(f'No crawl job or indexed source found for {job_or_source_id}')
...
def queue_indexed_source_crawl(self, source_id, force_full_hash=False):
    ...
    if source is None:
        raise IndexedSource.DoesNotExist(f'Indexed source not found or inactive: {source_id}')
    job = CrawlJobService.create_job(
        source,
        parameters={'trigger': 'scheduled', 'force_full_hash': bool(force_full_hash)},
    )
...
def queue_hash_backfill_job(self, batch_size=500, source_id=None, force_full_hash=False):
    source = None
    if source_id:
        source = IndexedSource.objects.filter(id=source_id, is_active=True).first()
        if source is None:
...
def index_single_document(self, document_id, force_full_hash=False):
    ...
    return DocumentIndexOrchestrator.index_document(document, force_hashes=True, force_full_hash=force_full_hash)
