...
router.register(r'indexing/hash-backfill-jobs', HashBackfillJobViewSet, basename='hash-backfill-job')
router.register(r'deduplication/decisions', DuplicateDecisionViewSet, basename='duplicate-decision')

urlpatterns = [
    path('', include(router.urls)),
    path('ocr/results/<str:document_id>/', OcrResultView.as_view(), name='ocr_result'),
    path('deduplication/groups/', DeduplicationGroupListView.as_view(), name='deduplication-group-list'),
    path('deduplication/groups/<path:group_key>/', DeduplicationGroupDetailView.as_view(), name='deduplication-group-detail'),
    path(
