...
class SearchFacetsView(APIView):
    ...
    def get(self, request):
        query = request.query_params.get('q', '').strip()
        return success_response(SearchService.facets_for_user(request.user, query=query))


class SavedQueryCreateView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        query = str(request.data.get('query', '')).strip()
        scope = str(request.data.get('scope', 'ALL')).upper()
        if not query:
            return error_response('query_required', 'query is required', status_code=status.HTTP_400_BAD_REQUEST)

        saved_query_id = uuid.uuid4().hex
        AuditService.log(
            'SEARCH',
            'System',
            user=request.user,
            entity=query,
            details={'scope': scope, 'saved_query_id': saved_query_id, 'saved': True},
            ip_address=request.META.get('REMOTE_ADDR'),
        )
        return success_response(
            {'id': saved_query_id, 'query': query, 'scope': scope},
