        ...
        <Card key={item.id}>
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div className="flex gap-1">
              <Button variant="ghost" size="icon" className="h-7 w-7" aria-label={`View ${item.duplicate}`}><Eye className="h-3.5 w-3.5" /></Button>
              <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" aria-label={`Delete ${item.duplicate}`}><Trash2 className="h-3.5 w-3.5" /></Button>
            </div>
          </CardContent>
        </Card>
      ))}
