  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon" className="h-7 w-7" asChild aria-label={`View document ${row.original.title}`} title={`View document ${row.original.title}`}>
          <Link href={`/documents/${row.original.id}`}><Eye className="h-3.5 w-3.5" /></Link>
        </Button>
        <Button variant="ghost" size="icon" className="h-7 w-7" asChild aria-label={`Edit document ${row.original.title}`} title={`Edit document ${row.original.title}`}>
          <Link href={`/documents/${row.original.id}/edit`}><Pencil className="h-3.5 w-3.5" /></Link>
        </Button>
        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" aria-label={`Delete document ${row.original.title}`} title={`Delete document ${row.original.title}`}>
